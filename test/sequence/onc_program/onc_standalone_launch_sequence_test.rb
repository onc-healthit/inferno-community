  ...
  describe 'ONC scopes test' do
    ...
    it 'fails when a required scope is missing' do
      @required_scopes.each do |scope|
        scopes = @required_scopes - [scope]
        @instance.instance_variable_set(:'@scopes', scopes.join(' '))
        exception = assert_raises(Inferno::AssertionException) { @sequence.run_test(@test) }

        assert_equal %(Scope did not include "#{scope}"), exception.message
      end
    end

    it 'fails when there is no patient-level scope' do
      @instance.instance_variable_set(:'@scopes', @required_scopes.join(' '))
      exception = assert_raises(Inferno::AssertionException) { @sequence.run_test(@test) }
    ...
    it 'fails when there is a badly formatted scope' do
      ...
      bad_scopes.each do |scope|
        @instance.instance_variable_set(:'@scopes', (@required_scopes + [scope]).join(' '))
        exception = assert_raises(Inferno::AssertionException) { @sequence.run_test(@test) }
      ...
      @instance.instance_variable_set(:'@scopes', (@required_scopes + ["patient/#{bad_resource_type}.*"]).join(' '))
      exception = assert_raises(Inferno::AssertionException) { @sequence.run_test(@test) }
    ...
    it 'succeeds when the required scopes and a patient-level scope are present' do
      @instance.instance_variable_set(:'@scopes', (@required_scopes + ['patient/*.*']).join(' '))
