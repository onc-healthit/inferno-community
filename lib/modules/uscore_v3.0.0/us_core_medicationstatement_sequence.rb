...
module Inferno
  module Sequence
    class USCore300MedicationstatementSequence < SequenceBase
      ...
      test :unauthorized_search do
        metadata do
          id '01'
          name 'Server rejects MedicationStatement search without authorization'
      ...
      test 'Server returns expected results from MedicationStatement search by patient' do
        ...
        save_delayed_sequence_references(@medicationstatement_ary)
      ...
      test 'Server returns expected results from MedicationStatement search by patient+effective' do
        ...
        patient_val = @instance.patient_id
        effective_val = get_value_for_search_param(resolve_element_from_path(@medicationstatement_ary, 'effectiveDateTime'))
      ...
      test 'Server returns expected results from MedicationStatement search by patient+status' do
        ...
        patient_val = @instance.patient_id
        status_val = get_value_for_search_param(resolve_element_from_path(@medicationstatement_ary, 'status'))
      ...
      test 'Server returns the appropriate resource from the following _includes: MedicationStatement:medication' do
        ...
        search_params['_include'] = 'MedicationStatement:medication'
        reply = get_resource_by_params(versioned_resource_class('MedicationStatement'), search_params)
        assert_response_ok(reply)
        assert_bundle_response(reply)
