...
module Inferno
  module Sequence
    class IpsBundleManualSequence < SequenceBase
      ...
      def run_test(test)
        @bundle = FHIR::Bundle.new(JSON.parse(@instance.ips_bundle_json)) if @bundle.nil?

        super(test)
      end

      resource_validate_bundle(index: '01')
      resource_validate_medication_statement(index: '02')
      resource_validate_allergy_intolerance(index: '03')
      resource_validate_condition(index: '04')
      resource_validate_composition(index: '05')
    end
  end
end
