    ...
    class USCore310PractitionerSequence < SequenceBase
      ...
      test :unauthorized_search do
        ...
        patient_ids.each do |patient|
          search_params = {
            'name': get_value_for_search_param(resolve_element_from_path(@practitioner_ary[patient], 'name'))
          }
      ...
      test :search_by_name do
        ...
        patient_ids.each do |patient|
          ...
          any_resources = reply&.resource&.entry&.any? { |entry| entry&.resource&.resourceType == 'Practitioner' }
          ...
          @practitioner = reply.resource.entry
            ...
            .resource
          @practitioner_ary[patient] = fetch_all_bundled_resources(reply.resource)
          save_resource_ids_in_bundle(versioned_resource_class('Practitioner'), reply)
          save_delayed_sequence_references(@practitioner_ary[patient])
      ...
      test :search_by_identifier do
        ...
        patient_ids.each do |patient|
          search_params = {
            'identifier': get_value_for_search_param(resolve_element_from_path(@practitioner_ary[patient], 'identifier'))
          }
      ...
      test 'All must support elements are provided in the Practitioner resources returned.' do
        ...
        skip_if missing_must_support_elements.present?,
                "Could not find #{missing_must_support_elements.join(', ')} in the #{@practitioner_ary&.values&.flatten&.length} provided Practitioner resource(s)"
      ...
      test 'Every reference within Practitioner resource is valid and can be read.' do
        ...
        @practitioner_ary&.values&.flatten&.each do |resource|
