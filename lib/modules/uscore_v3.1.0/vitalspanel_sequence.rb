...
module Inferno
  module Sequence
    class USCore310VitalspanelSequence < SequenceBase
      title 'Observation Vital Signs Panel Tests'

      description 'Verify that Observation resources on the FHIR server follow the US Core Implementation Guide'

      test_id_prefix 'USCOVSP'
      ...
      test 'Server returns expected results from Observation search by patient+code' do
        ...
        code_val.each do |val|
          ...
          save_resource_ids_in_bundle(versioned_resource_class('Observation'), reply, Inferno::ValidationUtil::US_CORE_R4_URIS[:vitals_panel])
      ...
      test 'Observation resources associated with Patient conform to US Core R4 profiles' do
        metadata do
          id '11'
          link 'http://hl7.org/fhir/StructureDefinition/vitalspanel'
          description %(
        ...
        skip 'No resources appear to be available for this patient. Please use patients with more information.' unless @resources_found
        test_resources_against_profile('Observation', Inferno::ValidationUtil::US_CORE_R4_URIS[:vitals_panel])
      end
