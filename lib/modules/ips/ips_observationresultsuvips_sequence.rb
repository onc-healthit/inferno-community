...
require_relative './profile_definitions/ips_observationresultsuvips_sequence_definitions'

module Inferno
  module Sequence
    class IpsObservationresultsuvipsSequence < SequenceBase
      ...
      test :resource_read do
        ...
        save_resource_references(versioned_resource_class('Observation'), [@resource_found], 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-results-uv-ips')
      end

      test :resource_validate_profile do
        metadata do
          id '02'
          name 'Server returns Observation resource that matches the Observation Results (IPS) profile'
          link 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-results-uv-ips'
          description %(
        ...
        test_resources_against_profile('Observation', 'http://hl7.org/fhir/uv/ips/StructureDefinition/Observation-results-uv-ips')
      end
      ...
      test :must_support do
        ...
        skip 'No resource found from Read test' unless @resource_found.present?
        must_supports = IpsObservationresultsuvipsSequenceDefinition::MUST_SUPPORTS
