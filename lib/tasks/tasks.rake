...
task :tests_to_csv do
  ...
  csv_out = CSV.generate do |csv|
    ...
    csv << ['', '', '', '', '']
    csv << ['Test ID', 'Extended Reference', 'Sequence/Group', 'Test Name', 'Required?', 'Description/Requirement', 'Reference URI']
    flat_tests.each do |test|
      csv <<  [test[:test_id], test[:ext_ref], test[:sequence], test[:name], test[:sequence_required] && test[:required], test[:description], test[:url] ]
    end
  end
