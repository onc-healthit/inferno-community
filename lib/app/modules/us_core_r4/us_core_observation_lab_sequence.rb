  ...
  module Sequence
    class USCoreR4ObservationLabSequence < SequenceBase
      ...
      test 'Server returns expected results from Observation search by patient+category' do
        ...
        save_resource_ids_in_bundle(versioned_resource_class('Observation'), reply, Inferno::ValidationUtil::US_CORE_R4_URIS[:observation_lab])
      ...
      test 'Observation resources associated with Patient conform to US Core R4 profiles' do
        ...
        skip 'No resources appear to be available for this patient. Please use patients with more information.' unless @resources_found
        test_resources_against_profile('Observation', Inferno::ValidationUtil::US_CORE_R4_URIS[:observation_lab])
      end
