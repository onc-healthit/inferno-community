...
def extract_metadata
  ...
  metadata = extract_metadata_from_resources(resources)
  ...
  resources.each do |resource|
    resource['supportedProfile'].each do |supported_profile|
      ...
      basic_searches&.each do |search_param|
        ...
        new_sequence[:searches] << new_search_param
        new_sequence[:search_param_descriptions][search_param['name'].to_sym] = {}
      end
      ...
      search_combos&.each do |combo|
        ...
        combo_params.each do |param|
          ...
          new_sequence[:search_param_descriptions][param['valueString'].to_sym] = {}
        end
        new_sequence[:searches] << new_search_combo
      end
      ...
      new_sequence[:search_param_descriptions].each_key do |param|
        search_param_definition = get_json_from_uri(search_param_uri(new_sequence[:resource], param.to_s))
        path_parts = search_param_definition['xpath'].split('/f:')
        if param.to_s != '_id'
          path_parts[0] = new_sequence[:resource]
        ...
        if !profile_element.nil?
          new_sequence[:search_param_descriptions][param][:type] = profile_element['type'].first['code']
          new_sequence[:search_param_descriptions][param][:path] = path
          new_sequence[:search_param_descriptions][param][:contains_multiple] = (profile_element['max'] == '*')
        else
          # search is a variable type eg.) Condition.onsetDateTime - element in profile def is Condition.onset[x]
          new_sequence[:search_param_descriptions][param][:type] = search_param_definition['type']
          new_sequence[:search_param_descriptions][param][:path] = path
          new_sequence[:search_param_descriptions][param][:contains_multiple] = false
        end
      end
      ...
      profile_definition['snapshot']['element'].each do |element|
        ...
        if path.include? '[x]'
          element['type'].each do |type|
            new_sequence[:element_descriptions][path.gsub('[x]', type['code']).downcase.to_sym] = { type: type['code'], contains_multiple: element['max'] == '*' }
          end
        else
          new_sequence[:element_descriptions][path.downcase.to_sym] = { type: element['type'].first['code'], contains_multiple: element['max'] == '*' }
        end
      end
