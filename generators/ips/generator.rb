...
def generate_sequence(sequence)
  ...
  file_name = OUT_PATH + '/ips/' + sequence[:name].downcase + '_sequence.rb'

  template = ERB.new(File.read('./templates/sequence.rb.erb'))
  output =   template.result_with_hash(sequence)
  FileUtils.mkdir_p(OUT_PATH + '/ips') unless File.directory?(OUT_PATH + '/ips')
...
def create_resource_profile_test(sequence)
  test = {
    tests_that: "#{sequence[:resource]} resources associated with Patient conform to IPS profiles",
    index: sequence[:tests].length + 1,
...
def generate_module(module_info)
  file_name = OUT_PATH + '/ips_module.yml'
