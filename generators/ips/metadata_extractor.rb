...
class MetadataExtractor
  ...
  def extract_metadata
    @metadata = extract_metadata_from_resources(['AllergyIntolerance', 'Condition'])
    add_special_cases
    @metadata
  end

  def build_new_sequence(resource, profile_json)
    profile_name = profile_json['name'].strip
    profile_title = profile_json['title'].strip

    {
      name: profile_name,
      classname: resource + 'Sequence',
      resource: resource,
      profile: profile_json['url'],
      title: profile_title,
      interactions: [],
      searches: [],
  ...
  def extract_metadata_from_resources(resources)
    ...
    resources.each do |resource|
      profile_definition = get_json_from_uri("StructureDefinition-#{resource}-uv-ips.json")

        new_sequence = build_new_sequence(resource, profile_definition)
